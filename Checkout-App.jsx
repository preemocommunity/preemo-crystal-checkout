...
const AGENTS = [
  ...
  { id:12, name:"The Crystal", title:"All Twelve. One Mind.",  icon:"∞", special:true },
];
...
function GlassCard({ children, style }) {
  return (
    <div style={{ background:B.glass, backdropFilter:"blur(20px)", WebkitBackdropFilter:"blur(20px)", border:`1px solid ${B.glassBorder}`, borderRadius:16, boxShadow:B.shadow, ...style }}>
      {children}
    </div>
  );
}
...
function AgentSelectCard({ agent, selected, disabled, onToggle }) {
  ...
  return (
    ...
    >
      <div style={{ width:18, height:18, borderRadius:4, flexShrink:0, border:`2px solid ${active?B.lightBlue:B.glassBorder}`, background:active?B.lightBlue:"transparent", display:"flex", alignItems:"center", justifyContent:"center", transition:"all 0.15s" }}>
  ...
  return (
    <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:10 }}>
      {BUNDLES.map(b => {
        ...
        return (
          <div key={b.count} onClick={() => onSelect(b.count)} style={{ border:`1.5px solid ${active?B.lightBlue:B.glassBorder}`, borderRadius:10, padding:"12px 10px", cursor:"pointer", textAlign:"center", background:active?`${B.lightBlue}09`:B.white, boxShadow:active?`0 0 0 3px ${B.lightBlue}18`:"none", transition:"all 0.18s", position:"relative" }}>
  ...
  return (
    <GlassCard style={{ padding:28, fontFamily:F }}>
              ...
              {selectedAgents.map(id => {
                ...
                return a ? (
                  <div key={id} style={{ display:"flex", alignItems:"center", gap:8 }}>
                    <div style={{ width:6, height:6, borderRadius:"50%", background:B.lightBlue, flexShrink:0 }}/>
                    <span style={{ fontSize:12, fontWeight:600, color:B.navy, fontFamily:F }}>{a.name}</span>
      ...
      <div style={{ background:B.skyBlue, borderRadius:10, padding:"12px 14px", marginBottom:20 }}>
        <div style={{ fontSize:9, fontWeight:700, letterSpacing:"1.5px", textTransform:"uppercase", color:B.midBlue, fontFamily:F, marginBottom:8 }}>What you receive</div>
        {[`${bundleCount} unique Fragment code${bundleCount>1?"s":""} — one per agent`,"Codes sent to your email instantly after payment","Enter each code on The Crystal to activate that agent","Codes never expire — lifetime access"].map(t => (
          <div key={t} style={{ display:"flex", gap:8, marginBottom:5 }}>
            <span style={{ fontSize:11, color:B.lightBlue, flexShrink:0 }}>✦</span>
            <span style={{ fontSize:11, color:B.midBlue, fontFamily:F, lineHeight:1.5 }}>{t}</span>
          </div>
        ))}
      </div>
      {!ready && (
        <div style={{ background:B.ice, border:`1px solid ${B.glassBorder}`, borderRadius:8, padding:"10px 14px", marginBottom:14, textAlign:"center" }}>
          <div style={{ fontSize:12, color:B.textMuted, fontFamily:F }}>Select {bundleCount-selectedAgents.length} more agent{bundleCount-selectedAgents.length!==1?"s":""} to continue</div>
        </div>
      )}
      <button onClick={onCheckout} disabled={loading||!ready} style={{ width:"100%", padding:"15px 0", background:!ready||loading?B.textLight:`linear-gradient(135deg,${B.navy} 0%,${B.midBlue} 100%)`, color:"#fff", border:"none", borderRadius:10, fontSize:15, fontWeight:800, fontFamily:F, cursor:!ready||loading?"default":"pointer", boxShadow:!ready||loading?"none":"0 6px 24px rgba(10,22,40,0.3)", transition:"all 0.2s" }}>
      ...
      <div style={{ marginTop:14, display:"flex", flexDirection:"column", gap:6 }}>
        {["One-time · No subscription ever","Secured by Stripe · PCI compliant","30-day money-back guarantee"].map(t => (
          <div key={t} style={{ display:"flex", alignItems:"center", gap:7 }}>
            ...
            <span style={{ fontSize:11, color:B.textLight, fontFamily:F }}>{t}</span>
          </div>
        ))}
      </div>
    </GlassCard>
...
export default function App() {
  ...
  return (
    <div style={{ fontFamily:F, minHeight:"100vh", background:`linear-gradient(165deg,${B.white} 0%,${B.ice} 40%,${B.skyBlue} 100%)`, color:B.text }}>
      ...
      <div style={{ position:"fixed",bottom:"-8%",left:"-4%",width:380,height:380,borderRadius:"50%",background:"radial-gradient(circle,rgba(10,22,40,0.04) 0%,transparent 70%)",pointerEvents:"none",zIndex:0 }}/>
      ...
      <div style={{ position:"relative",zIndex:1,maxWidth:1060,margin:"0 auto",padding:"52px 24px 80px" }}>
        <div className="fadein" style={{ textAlign:"center",marginBottom:48 }}>
          ...
          <p style={{ fontSize:16,color:B.textMuted,lineHeight:1.75,maxWidth:500,margin:"0 auto",fontFamily:F,fontWeight:500 }}>Each Fragment unlocks one AI agent inside The Crystal vault. Choose how many, pick which agents. One unique code per agent, delivered instantly.</p>
        </div>
        ...
        <div className="grid" style={{ display:"grid",gridTemplateColumns:"1fr 360px",gap:32,alignItems:"start" }}>
          <div style={{ display:"flex",flexDirection:"column",gap:24 }}>
            ...
            <GlassCard style={{ padding:28 }}>
              ...
              <div style={{ fontSize:12,color:B.textMuted,fontFamily:F,marginBottom:16,lineHeight:1.6 }}>Select {bundleCount} agent{bundleCount>1?"s":""}. A unique Fragment code will be generated for each one and sent to your email.</div>
            ...
            <GlassCard style={{ padding:28 }}>
              ...
              <div style={{ display:"flex",flexDirection:"column",gap:16 }}>
                {[["1","Choose fragments & agents","Select your bundle and pick exactly which agents you want."],["2","Pay once via Stripe","Secure one-time payment. No subscription. No recurring charge."],["3","Receive your Fragment codes","One CRYSTAL-XXXX-XXXX code per agent, sent instantly to your email."],["4","Activate on The Crystal","Enter each code at The Crystal to unlock and talk to that agent."],["5","Come back for more","Buy additional fragments anytime. They accumulate under your profile."]].map(([n,title,desc]) => (
            ...
            <GlassCard style={{ padding:28 }}>
              ...
              {[["Is this really a one-time payment?","Yes. One payment, lifetime access. No subscription, no renewal, no expiry. Ever."],["What is a Fragment code?","A code in the format CRYSTAL-XXXX-XXXX. Each is unique, cryptographically generated, and linked to one specific agent under your account."],["Can I buy more fragments later?","Yes. Come back anytime and buy more. All fragments accumulate under your profile on The Crystal."],["What if I lose my codes?","Your codes are permanently linked to your email. Log into The Crystal and all your codes and activated agents will be there."],["Do I have to pick all agents now?","You pick which agents to unlock with each purchase. Buy another bundle later to unlock different ones."]].map(([q,a],i,arr) => (
